...
# Version officielle selon la documentation listmonk.app

set -e

echo "🇫🇷 Intégration des mairies dans Listmonk Docker (Version officielle)"
echo "===================================================================="
...
# Tester l'authentification selon la documentation officielle
test_official_auth() {
    log_info "Test de l'authentification selon la documentation officielle..."
    
    # Méthode 1: BasicAuth (recommandée par la doc)
    log_info "Test BasicAuth: curl -u \"$API_USER:$API_TOKEN\""
    auth_response=$(curl -s -u "$API_USER:$API_TOKEN" "$API_URL/lists")
    
    if echo "$auth_response" | grep -q '"data"'; then
        log_success "✅ BasicAuth fonctionne"
        AUTH_METHOD="basic"
        return 0
    else
        log_warning "BasicAuth échoué: $auth_response"
    fi
    
    # Méthode 2: Authorization token header
    log_info "Test Authorization token: curl -H \"Authorization: token $API_USER:$API_TOKEN\""
    auth_response=$(curl -s -H "Authorization: token $API_USER:$API_TOKEN" "$API_URL/lists")
    
    if echo "$auth_response" | grep -q '"data"'; then
        log_success "✅ Authorization token fonctionne"
        AUTH_METHOD="token"
        return 0
    else
        log_warning "Authorization token échoué: $auth_response"
    fi
    
    log_error "❌ Aucune méthode d'authentification ne fonctionne"
    log_info "Réponses reçues :"
    log_info "BasicAuth: $auth_response"
    return 1
}
...
import_data_via_api() {
    ...
    # Construire la commande curl selon la méthode d'authentification
    if [ "$AUTH_METHOD" = "basic" ]; then
        auth_param="-u $API_USER:$API_TOKEN"
        log_info "🔐 Utilisation de BasicAuth"
    elif [ "$AUTH_METHOD" = "token" ]; then
        auth_param="-H \"Authorization: token $API_USER:$API_TOKEN\""
        log_info "🔐 Utilisation du token Authorization"
    else
    ...
    log_info "📤 Import du fichier CSV via l'API import/subscribers..."
    
    # Utiliser l'endpoint d'import de Listmonk selon la documentation officielle
    # Paramètres JSON selon la doc : {"mode":"subscribe", "delim":",", "lists":[], "overwrite": true}
    ...
    if [ "$AUTH_METHOD" = "basic" ]; then
        ...
        import_response=$(curl -s -H "Authorization: token $API_USER:$API_TOKEN" -X POST \
            -F "params=$import_params" \
            -F "file=@mairielist-converted.csv" \
            "$API_URL/import/subscribers")
    fi
        ...
        if echo "$import_response" | grep -q '"data"'; then
            ...
            for i in {1..20}; do
                if [ "$AUTH_METHOD" = "basic" ]; then
                    ...
                    status_response=$(curl -s -H "Authorization: token $API_USER:$API_TOKEN" "$API_URL/import/subscribers")
                fi
...
verify_integration() {
    ...
    if [ "$AUTH_METHOD" = "basic" ]; then
        ...
        subscribers_response=$(curl -s -H "Authorization: token $API_USER:$API_TOKEN" "$API_URL/subscribers?per_page=1")
    fi
    
    if [ $? -eq 0 ]; then
        # Extraire le nombre total d'abonnés selon la structure officielle
    ...
    if [ "$AUTH_METHOD" = "basic" ]; then
        recent_response=$(curl -s -u "$API_USER:$API_TOKEN" "$API_URL/subscribers?order_by=created_at&order=desc&per_page=3")
    else
        recent_response=$(curl -s -H "Authorization: token $API_USER:$API_TOKEN" "$API_URL/subscribers?order_by=created_at&order=desc&per_page=3")
    fi
...
show_final_info() {
    ...
    if [ "$AUTH_METHOD" = "basic" ]; then
        ...
        echo "   curl -H \"Authorization: token $API_USER:$API_TOKEN\" $API_URL/subscribers"
        echo "   curl -H \"Authorization: token $API_USER:$API_TOKEN\" $API_URL/lists"
    fi
    echo ""
}
...
main() {
    ...
    echo ""
    echo "🔑 Utilisation des identifiants officiels :"
    echo "   Utilisateur : $API_USER"
    echo "   Token : $API_TOKEN"
    echo ""
    echo "Étapes :"
    echo "  1. Vérifier l'environnement Docker"
    echo "  2. Tester l'authentification selon la doc officielle"
